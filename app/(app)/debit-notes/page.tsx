...
import { Plus, FilePlus, Search, Eye } from 'lucide-react';
...
export default function DebitNotesPage() {
  ...
  function create() {
    ...
    toast.success('Nota débito creada', { description: `Asociada a ${facturaOrigen} · ${motivo}.` });
    setSheetOpen(false);
    setFacturaOrigen('');
  }
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Notas débito"
        description="Notas débito asociadas a facturas de venta existentes."
        actions={
          <Button onClick={() => setSheetOpen(true)}>
            <Plus className="mr-2 h-4 w-4" /> Nueva nota débito
          </Button>
        }
      />
      ...
      {filtered.length === 0 ? (
        <EmptyState
          ...
          description="Crea una nota débito asociada a una factura existente."
          actionLabel="Nueva nota débito"
        ...
        <SheetContent side="right" className="w-full sm:max-w-md">
          <SheetHeader>
            <SheetTitle className="flex items-center gap-2">
              <FilePlus className="h-4 w-4 text-primary" /> Nueva nota débito
            </SheetTitle>
          </SheetHeader>
          ...
          <SheetFooter className="mt-6">
            ...
            <Button onClick={create}>Crear nota débito</Button>
          </SheetFooter>
        </SheetContent>
      </Sheet>
    </div>
